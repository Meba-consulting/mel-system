...
@Component({
  ...
  styleUrls: ['./human-resource-management.component.css']
})
export class HumanResourceManagementComponent implements OnInit {
          ...
          if (this.selectedProgram.id.indexOf('default') > -1) {
            ...
            this.dataElements$.subscribe(elements => {
              if (elements) {
                ...
                this.entityAttributeHeaders = _.take(
                  _.union(
                    this.entityAttributeHeaders,
                    filterWithContainingCharactes(
                      elements['trackedEntityAttributes'],
  ...
  onSelectBatch(item) {
    ...
    console.log('dgdg', this.dataEntrySelections);
    this.selectedTrackedEntityInstance = item.id;
    this.dataEntrySelections.batchNo = item.name;
    console.log('dgdg 2', this.dataEntrySelections);
