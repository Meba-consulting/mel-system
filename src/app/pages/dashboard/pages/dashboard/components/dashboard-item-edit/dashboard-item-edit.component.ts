...
export class DashboardItemEditComponent implements OnInit {
  ...
  constructor(private dialogRef: MatDialogRef<DashboardItemEditComponent>, @Inject(MAT_DIALOG_DATA) data, private httpClient: NgxDhis2HttpClientService) {
  ...
  onSave(e) {
    ...
    e.stopPropagation();
    this.updateResponse$ = this.httpClient.put('dashboards/' + this.dashboard?.id+'?mergeMode=UPDATE', data).pipe(map(response => {
      this.saving = false;
      this.changed = true;
      return response
    }),catchError(e => {
